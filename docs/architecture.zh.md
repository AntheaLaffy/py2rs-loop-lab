...
每个单元在实现前选择一个 R0 oracle：

- `behavior_parity`：默认；legacy Python public seam 是 oracle，由 behavior reviewer 证明强行为一致性。
- `rust_compatibility`：只用于已声明的深层 framework/runtime 边界；oracle 是已有强一致性证据的 canonical Rust contracts，由 compatibility reviewer 证明应用兼容。

例如 Python/Rust 深度学习框架对 tensor shape、dtype、layout 或 artifact 的解释不同，这些差异会穿透到 codec、模型配置和权重加载，但复刻整个 Python framework 不在重写范围内。此时可以在深度推理链入口切换到 `rust_compatibility`，明确测试 tensor handoff、编解码、模型加载、schema 和错误投影，同时排除 framework 内部实现。

这个切换必须在 writer 前记录 rationale、已验证 Rust unit/report、required contracts 和 excluded legacy internals。它不能由一次 parity 失败自动触发，不能把新单元自己当 oracle，也不能虚假声称 Python parity。
