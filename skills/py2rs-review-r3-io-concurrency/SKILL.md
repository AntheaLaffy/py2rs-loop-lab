...
description: "[DRAFT] 第 3 轮 IO、并发与运行时人体工学审查。检查阻塞 IO、async 边界、取消、重试、并发限制和运行时嵌套；默认审查不改生产代码。"
---
...
- The unit's runtime context is known: CLI, service, Tauri, Python bridge, batch worker or library.
...
1. Confirm unit id and runtime environment.
...
- Require R0 behavior rerun after changes.
...
```text
reviews/YYYY-MM-DD-<unit-id>-async-ergonomics.md
```
