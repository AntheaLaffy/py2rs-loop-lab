...
description: "[DRAFT] 第 2 轮错误与追踪审查。检查结构化错误、上下文、日志、trace 传播和敏感信息保护；默认审查不改生产代码，修复后必须重跑行为门禁。"
---
...
- The unit's public error contract is known.

## Review Focus

- Public errors preserve legacy semantics until protocol changes are explicitly approved.
...
1. Confirm unit id, seam and public error policy.
...
```text
reviews/YYYY-MM-DD-<unit-id>-error-tracing.md
```
