...
4. 初始化重写工作区：mission、resources、notes、records、manifest、重写/框架偏好、behavior verification policy、依赖源码策略、granularity、review cadence、manifest partitioning 和 execution policy。
...
- **验证目标**：每个单元都以明确的 legacy public seam 作为行为 oracle。R0 证明 Python/Rust 可观察行为一致；默认精确比较，模型或数值结果只有在公共契约或实现前明确记录容差时才能例外。深层 framework 如果无法提供可独立比较的 seam，就重切单元或保留 legacy owner。
...
| 显式 behavior seam | 每个单元在实现前记录 legacy public seam 和可观察用例。tensor、codec、artifact、模型加载和 handoff 只要穿透该 seam 就纳入 R0；不会影响 seam 的 framework 内部细节不作为重写目标。 |
...
所以目标不是“流程越多越严谨”，而是只在需要判断的地方花 token，把稳定步骤固化，让 writer 连续推进并提升整体重写速度，把审核频率交给用户，同时用 canonical dependency、R0、逐单元证据和 promotion 约束守住一致性；seam 和显式比较容差必须在实现前记录。
...
- `manifest` / 清单：记录每个迁移单元的状态、owner、目标 owner、behavior verification seam、验证命令、回滚路径、必需审核门、审核频率、分片与执行策略。它是重写进度的控制面。
...
- 依赖对齐：先应用已记录的重写/框架偏好，再按能力判断 Python 依赖、Rust crate、semantic-delta adapter 或手写实现如何覆盖同一行为。
- bootstrap：在迁移业务逻辑前，先证明选定 seam 能跑通参数、返回值、错误、日志、测试和回滚路径。
- review evidence：一个批次可以共享审核上下文，但必须逐单元给 verdict；单元通过 R0 behavior 和必要的 R1-R6 后才能推进。

这套架构的意义是让迁移在任何时刻都保持可运行、可测试、可回滚。即使换一个 AI 会话，也能通过仓库里的 mission、records、manifest 和 reviews 接着做，而不是重新猜项目状态。

### 初始化偏好与依赖引入时机

这次的初始化架构把用户意图和迁移状态分开管理：先记录总体重写策略和相关框架偏好，同时记录 crate 侦察模式与代理。初始化还会询问每几个单元总审、是否分片，并把执行默认设为 serial；每个单元在实现前记录 legacy public seam 和比较策略。长期依赖偏好写入 `NOTES.md`，verification/review/execution policy、owner、验证命令和回滚路径由 manifest 管理。
...
使用这套 skills 迁移 Python 到 Rust 后，第一阶段的目标不是逐行翻译 Python，也不是立刻写出最 idiomatic 的 Rust。每个单元都要在声明的 legacy public seam 上证明强行为一致，并配套覆盖面完整的测试；不会穿透该 seam 的深层 framework 内部不属于重写范围。

也就是说，迁移后的 Rust 代码首先应该证明：

- 每个单元在声明的 seam 上保持原 Python 版本的对外行为。
- 只要穿透 seam，tensor handoff、编解码、artifact 和模型加载就必须纳入行为测试。
