...
这次的初始化架构把用户意图和迁移状态分开管理：先通过两阶段询问记录总体重写策略，再只询问项目实际涉及的框架类别。长期偏好写入 `NOTES.md`；单元 owner、验证命令和回滚路径继续由 manifest 管理。

初始化阶段不预装尚未证明需要的 crate。等 seam 或迁移单元进入依赖对齐后，py2rs 才把已记录偏好与项目事实、行为 fixtures 和当前依赖证据结合起来，只加入并锁定该单元真正需要的依赖。单元记录会说明偏好如何被应用或为何必须调整，bootstrap 则在业务逻辑迁移前验证最终选定的技术栈。
